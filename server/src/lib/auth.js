...
export function destroyTeacherSession(t) { q.run(`DELETE FROM teacher_sessions WHERE token=?`, t); }
...
export function requireCourseAccess(req, res, next) {
  const courseId = Number(req.params.id || req.params.courseId);
  const acc = canAccessCourse(req.teacher, courseId);
...
export function createStudentSession(studentId, examId, attemptId = null) {
  // enforce one active session per student
  q.run(`UPDATE student_sessions SET active = 0 WHERE student_id = ? AND active = 1`, studentId);
  const t = token();
  q.run(
...
export function attachAttemptToStudentSession(t, attemptId) {
  q.run(`UPDATE student_sessions SET attempt_id = ? WHERE token = ?`, attemptId, t);
}
