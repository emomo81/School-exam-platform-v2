...
authRouter.post('/teacher/login', async (req, res) => {
  ...
  const t = await q.get(`SELECT * FROM teachers WHERE email = ?`, String(email || '').trim().toLowerCase());
  if (!t || !checkPassword(String(password || ''), t.password_hash)) {
    return bad(res, 'Invalid email or password', 401);
  }
  const token = await createTeacherSession(t.id);
  res.cookie(cookies.TEACHER_COOKIE, token, cookieOpts(72 * 3600e3));
  await audit('teacher', t.id, 'auth.login', 'teacher', t.id);
...
authRouter.post('/teacher/register', async (req, res) => {
  ...
  if (await q.get(`SELECT 1 AS x FROM teachers WHERE email = ?`, em)) return bad(res, 'Email already registered', 409);
  ...
  res.cookie(cookies.TEACHER_COOKIE, token, cookieOpts(72 * 3600e3));
  await audit('teacher', teacherId, 'auth.registered', 'teacher', teacherId);
  res.status(201).json({ id: teacherId, name: name.trim(), email: em, role: 'teacher', token });
});

authRouter.post('/teacher/logout', async (req, res) => {
  const t = req.cookies?.[cookies.TEACHER_COOKIE] || bearerOf(req);
  if (t) await destroyTeacherSession(t);
...
authRouter.post('/teacher/change-password', requireTeacher, async (req, res) => {
  ...
  const t = await q.get(`SELECT * FROM teachers WHERE id = ?`, req.teacher.id);
  ...
  await q.run(`UPDATE teachers SET password_hash = ? WHERE id = ?`, hashPassword(String(next)), t.id);
  await audit('teacher', t.id, 'auth.password_changed', 'teacher', t.id);
  res.json({ ok: true });
});
...
authRouter.post('/student/login', async (req, res) => {
  ...
  const exam = await q.get(`SELECT e.*, c.code AS course_code, c.title AS course_title, c.term
  ...
  const student = await q.get(`SELECT * FROM students WHERE UPPER(roll_no) = ?`, rollNo);
  if (!student) return bad(res, 'Roll number not found on the roster', 403);

  const onRoster = exam.use_roster_override
    ? await q.get(`SELECT 1 AS ok FROM exam_roster_overrides WHERE exam_id = ? AND student_id = ?`, exam.id, student.id)
    : await q.get(`SELECT 1 AS ok FROM enrollments WHERE course_id = ? AND student_id = ?`, exam.course_id, student.id);
  if (!onRoster) {
    await audit('student', student.id, 'auth.denied_not_on_roster', 'exam', exam.id);
    return bad(res, 'Your roll number is not on the roster for this exam. Contact your instructor.', 403);
  }

  const status = examStatus(exam);
  const existing = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND student_id = ?`, exam.id, student.id);

  const token = await createStudentSession(student.id, exam.id, existing?.id ?? null);
  res.cookie(cookies.STUDENT_COOKIE, token, cookieOpts(12 * 3600e3));
  await audit('student', student.id, 'auth.student_login', 'exam', exam.id);
