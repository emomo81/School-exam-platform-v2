...
import { q, nowIso, USE_PG } from '../db/index.js';
...
miscRouter.get('/teachers', async (req, res) => {
  res.json(await q.all(`SELECT id, name, email, role FROM teachers ORDER BY name`));
});
...
miscRouter.get('/me/badges', async (req, res) => {
  const t = await q.get(`SELECT activity_seen_at, ai_seen_at FROM teachers WHERE id = ?`, req.teacher.id);
  ...
  const notifications = (await q.get(
...
miscRouter.post('/me/seen', async (req, res) => {
  ...
  const at = nowIso();
  await q.run(`UPDATE teachers SET ${col} = ? WHERE id = ?`, at, req.teacher.id);
  res.json({ ok: true, target, at });
});

// System status (integrations page)
miscRouter.get('/system/status', async (req, res) => {
  const counts = {
    courses: (await q.get(`SELECT COUNT(*) AS n FROM courses`)).n,
    exams: (await q.get(`SELECT COUNT(*) AS n FROM exams`)).n,
    students: (await q.get(`SELECT COUNT(*) AS n FROM students`)).n,
    attempts: (await q.get(`SELECT COUNT(*) AS n FROM attempts`)).n,
  };
  res.json({
    ok: true,
    version: '1.0.0',
    db: USE_PG ? 'postgres (pg) — Supabase' : 'sqlite (better-sqlite3) — WAL',
