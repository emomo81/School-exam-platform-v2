...
async function enforceClock(req, res) {
  const attempt = req.student.attemptId ? await getAttempt(req.student.attemptId) : null;
  if (!attempt) return { attempt: null };
  if (attempt.status === 'in_progress' && Date.now() >= Date.parse(attempt.ends_at)) {
    await finalizeAttempt(attempt.id, 'auto');
    return { attempt: await getAttempt(attempt.id), expired: true };
  }
  return { attempt };
}
...
studentRouter.get('/state', async (req, res) => {
  const exam = await loadExam(req);
  if (!exam) return bad(res, 'Exam not found', 404);
  const { attempt } = await enforceClock(req, res);
  res.json({
    student: req.student,
    exam: {
...
studentRouter.post('/start', async (req, res) => {
  ...
  let attempt = req.student.attemptId ? await getAttempt(req.student.attemptId) : null;
  if (!attempt) attempt = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND student_id = ?`, exam.id, req.student.id);
  if (attempt && attempt.status !== 'in_progress') {
    return bad(res, 'You have already submitted this exam.', 409);
  }
  if (!attempt) {
    try {
      attempt = await createAttempt(exam, req.student.id);
    } catch (e) {
      if (String(e.message).includes('UNIQUE') || String(e.message).includes('duplicate key')) {
        attempt = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND student_id = ?`, exam.id, req.student.id);
      } else throw e;
    }
  }
  await attachAttemptToStudentSession(req.student.token, attempt.id);
  ssePublish(exam.id, 'attempt', { type: 'attempt', attemptId: attempt.id, status: 'in_progress' });
  res.json({
    attempt: attemptState(attempt),
    paper: await buildPaper(attempt),
    saved: await savedAnswersFor(attempt.id),
    server_now: nowIso(),
  });
});

studentRouter.get('/paper', async (req, res) => {
  const { attempt } = await enforceClock(req, res);
  ...
  res.json({ attempt: attemptState(attempt), paper: await buildPaper(attempt), saved: await savedAnswersFor(attempt.id), server_now: nowIso() });
});

// Answer autosave (batched). Displayed option indices are converted server-side.
studentRouter.post('/sync', async (req, res) => {
  const { attempt } = await enforceClock(req, res);
  if (!attempt || attempt.status !== 'in_progress') {
    return res.status(409).json({ expired: true, attempt: attemptState(attempt) });
  }
  await saveAnswers(attempt, req.body?.answers || []);
...
studentRouter.post('/heartbeat', async (req, res) => {
  const { attempt, expired } = await enforceClock(req, res);
  if (!attempt) return res.status(409).json({ expired: true });
  if (attempt.status === 'in_progress') {
    await q.run(`UPDATE attempts SET last_seen = ? WHERE id = ?`, nowIso(), attempt.id);
  }
  res.json({ attempt: attemptState(await getAttempt(attempt.id)), server_now: nowIso(), expired: !!expired });
});

// Violation reporting from the lockdown client (PRD 4.7).
studentRouter.post('/violation', async (req, res) => {
  const attempt = req.student.attemptId ? await getAttempt(req.student.attemptId) : null;
  if (!attempt || attempt.status !== 'in_progress') {
    return res.json({ ignored: true, attempt: attemptState(attempt) });
  }
  const result = await recordViolation(attempt, req.body?.type, req.body?.detail);
  res.json({ ...result, attempt: attemptState(await getAttempt(attempt.id)) });
});
...
studentRouter.post('/submit', async (req, res) => {
  ...
  const final = attempt.status === 'in_progress' ? await finalizeAttempt(attempt.id, 'submitted') : attempt;
...
studentRouter.get('/results', async (req, res) => {
  const exam = await loadExam(req);
  if (!exam) return bad(res, 'Exam not found', 404);
  const attempt = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND student_id = ?`, exam.id, req.student.id);
  ...
  const items = [];
  for (const [idx, o] of order.entries()) {
    const qu = await q.get(`SELECT * FROM questions WHERE id = ?`, o.question_id);
    const an = await q.get(`SELECT * FROM answers WHERE attempt_id = ? AND question_id = ?`, attempt.id, o.question_id);
    const base = { position: idx + 1, type: qu.type, points: qu.points, correct: null, score: null, status: 'unanswered' };
    if (qu.type === 'mcq') {
  ...
  for (const e of courseExams) {
    const a = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND student_id = ?`, e.id, req.student.id);
    if (!a || !e.results_released || !a.max_score || a.status === 'in_progress') continue;
    cumulative.push({ exam_id: e.id, title: e.title, pct: fmtPct(a.score || 0, a.max_score), score: a.score, max: a.max_score });
  }
