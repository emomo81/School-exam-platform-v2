...
aiRouter.post('/courses/:id/notes', upload.single('file'), async (req, res, next) => {
  try {
    ...
    await audit('teacher', req.teacher.id, 'notes.uploaded', 'course', acc.course.id, { filename: req.file.originalname, chars: text.length });
    res.status(201).json(await q.get(`SELECT id, course_id, filename, chars, created_at FROM notes WHERE id = ?`, info.lastInsertRowid));
  } catch (e) { next(e); }
});

aiRouter.delete('/notes/:id', async (req, res) => {
  const acc = await noteAccess(req.teacher, req.params.id);
  ...
  await q.run(`DELETE FROM notes WHERE id = ?`, acc.note.id);
  res.json({ ok: true });
});
...
aiRouter.post('/ai/generate', async (req, res, next) => {
  try {
    ...
    const acc = await canAccessCourse(req.teacher, course_id);
    if (!acc) return bad(res, 'Course not found', 404);
    let exam = null;
    if (exam_id) {
      const ea = await examAccess(req.teacher, exam_id);
      if (!ea || ea.exam.course_id !== acc.course.id) return bad(res, 'Exam not in this course', 404);
      exam = ea.exam;
    }
    const noteRows = note_id
      ? await q.all(`SELECT * FROM notes WHERE id = ? AND course_id = ?`, Number(note_id), acc.course.id)
      : await q.all(`SELECT * FROM notes WHERE course_id = ? ORDER BY id DESC LIMIT 5`, acc.course.id);
    ...
    await audit('teacher', req.teacher.id, 'ai.questions_generated', 'course', acc.course.id,
...
// ------------------------------- Review queue --------------------------------
aiRouter.get('/ai/queue', async (req, res) => {
  const courseIds = [
  ...
  const essayPending = (await q.get(
  ...
  const flagged = (await q.get(
    `SELECT COUNT(*) AS n FROM (
     ...
     ) AS flagged_questions`, ...courseIds, ...courseIds
  )).n;
...
aiRouter.post('/ai/queue/:id/approve', async (req, res, next) => {
  try {
    const g = await q.get(`SELECT * FROM ai_generations WHERE id = ?`, req.params.id);
    if (!g) return bad(res, 'Queue item not found', 404);
    const acc = await canAccessCourse(req.teacher, g.course_id);
    ...
    if (g.kind === 'rubric') {
      ...
      await q.run(`UPDATE ai_generations SET status = 'approved', reviewed_by = ?, reviewed_at = ? WHERE id = ?`,
    ...
    let questions;
    if (targetExamId) {
      const ea = await examAccess(req.teacher, targetExamId);
      if (!ea || ea.exam.course_id !== g.course_id) return bad(res, 'Target exam not in this course', 404);
      if ((await q.get(`SELECT COUNT(*) AS n FROM attempts WHERE exam_id = ?`, ea.exam.id)).n > 0) {
      ...
      const bank = await q.get(`SELECT * FROM question_banks WHERE id = ? AND course_id = ?`, Number(targetBankId), g.course_id);
    ...
    await q.run(`UPDATE ai_generations SET status = 'approved', reviewed_by = ?, reviewed_at = ? WHERE id = ?`, req.teacher.id, nowIso(), g.id);
    await audit('teacher', req.teacher.id, 'ai.question_approved', 'ai_generation', g.id, { question_id: questions });
    res.json({ ok: true, question_id: questions });
  } catch (e) { next(e); }
});

aiRouter.post('/ai/queue/:id/reject', async (req, res) => {
  const g = await q.get(`SELECT * FROM ai_generations WHERE id = ?`, req.params.id);
  if (!g) return bad(res, 'Queue item not found', 404);
  const acc = await canAccessCourse(req.teacher, g.course_id);
  if (!acc || !requireNonTa(acc.role)) return bad(res, 'Not permitted', 403);
  await q.run(`UPDATE ai_generations SET status = 'rejected', reviewed_by = ?, reviewed_at = ? WHERE id = ?`, req.teacher.id, nowIso(), g.id);
  await audit('teacher', req.teacher.id, 'ai.question_rejected', 'ai_generation', g.id);
  res.json({ ok: true });
});
