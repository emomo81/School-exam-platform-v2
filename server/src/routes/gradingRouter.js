...
gradingRouter.get('/exams/:id/grading', (req, res) => {
  ...
  const byQ = {};
  for (const qu of q.all(`SELECT id, text FROM questions WHERE exam_id = ? AND type = 'essay'`, acc.exam.id)) {
    byQ[qu.id] = qu.text;
  }
  res.json({
...
gradingRouter.post('/answers/:id/grade', (req, res) => {
  const row = q.get(
  ...
  const acc = examAccess(req.teacher, row.exam_id);
  if (!acc) return bad(res, 'Not permitted', 403);
  const score = Number(req.body?.score);
  const qu = q.get(`SELECT * FROM questions WHERE id = ?`, row.question_id);
  ...
  if (isOverride) {
    ...
    audit('teacher', req.teacher.id, 'grade.override', 'answer', row.id,
      { ai_score: row.ai_score, teacher_score: score, attempt_id: row.att_id });
  } else {
    audit('teacher', req.teacher.id, 'grade.confirmed', 'answer', row.id, { score, attempt_id: row.att_id });
  }
  recomputeScore(row.att_id);
  res.json({ ok: true, status: txStatus });
});

// Re-run (or run) Gemini grading for all ungraded/pending essay answers of an exam.
gradingRouter.post('/exams/:id/ai-grade', (req, res) => {
  const acc = examAccess(req.teacher, req.params.id);
  if (!acc) return bad(res, 'Exam not found', 404);
  const rows = q.all(
  ...
  for (const r of rows) {
    q.run(`UPDATE answers SET grading_status = 'ai_pending', updated_at = ? WHERE id = ?`, nowIso(), r.id);
  ...
  audit('teacher', req.teacher.id, 'ai.grading_requested', 'exam', acc.exam.id, { count: rows.length });
